...
plus.cdf <- function(y, support,h ,dh) 
   {
       zed <- c(min, z(support, h, dh), max)
...
s_k_sample = function(support, h, dh)
{
  zed = z(support, h, dh)
...
ars <- function(n, f, min, max, sp = starting_x(f, min, max)){
  
  
  check <- logconcav_check(f, seq(min, max, by = 0.1))
  if (!check) { print("The functino isn't log concave. Please input a log concave function")} else{
  g_func <- function(input_func, x){
    # docstring:
    # input_func is the underlying density function
    # x is where you want to evaluate
