...
ars <- function(n, f, min, max, sp = c(-Inf, Inf)){
    ...
    g_func <- function(f, x){
      ...
      if(sum(is.finite(sp)) < length(sp)) {sp = starting_x(f, min, max)}
...
ars(10, f = function(x) dnorm(x), -10, 10, c(-Inf, Inf)) #infinite loop, xstar never accepted
