...
logconcav_check <- function(x){ #x is domain given
  # docstring
  ...
  if(x[1] == -Inf) {x[1] = -.Machine$integer.max}
  if(x[length(x)] == Inf) {x[length(x)] = .Machine$integer.max}
  x[which(x == 0)] <- .Machine$double.eps
  x[1] <- ifelse(x[1] > 0, log(x[1]), -log(abs(x[1])))
  x[length(x)] <- ifelse(x[length(x)] > 0, log(x[length(x)]), -log(abs(x[length(x)])))
  seq_x <- seq(x[1], x[length(x)], by = 0.001)
  result <- numeric_sec_deri(h, x = seq_x)
  result[which(is.nan(result))] <- -.Machine$integer.max
  l <- 0
...
plus.cdf <- plus.cdf <- function(y, support)
{
  zed <- c(-Inf, z(support), Inf)
...
ars <- function(n, f, min, max, sp = NaN){
    ...
    g_func <- function(f, x){
      ...
      h <- function(x) {log(g_func(f,x))}
      dh <- function(x) {fderiv(h, x, 1)}   
      check <- logconcav_check(f, c(min,max))
      if (!check) { print("The function isn't log concave. Please input a log concave function")} 
      else {
...
ars(10, f = function(x) dnorm(x,-5), -10, 10, c(-8, 2))
ars(20, f = function(x) dnorm(x), -10, 10, c(-1, 1))
ars(20, f = function(x) dnorm(x), -10, 10, c(-1, 1))
ars(20, f = function(x) dnorm(x), -20, 30, c(-19, 1))

ars(20, f = function(x) dnorm(x), -20, 30, )
# not working 
ars(20, f = function(x) dnorm(x, 25, 3), -5, 45, c(-3, 37))

# non log concave function input
ars(20, f = function(x) x^2, -10, 10, c(-8, 2)) # gives us the correct error
```
