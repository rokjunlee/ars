...
#3. MIN and MAX of the DOMAIN
#4. (OPTIONAL) STARTINGPOINTS EG C(1,2)
...
ars <- function(n, f, min, max, sp = NA){
  ...
  if (prod(dh(c(min, max))) == 1) { sample <- runif(n, min, max)}
    else {
...
```{r, checking chunk}
# Checking
ars(300, f = function(x) dnorm(x, 2), -10, 10, c(-2, 3))
ars(300, f = function(x) dnorm(x, -1), -10, 10, c(-2, 1))
ars(300, f = function(x) dnorm(x,-5), -10, 10, c(-8, 2)) 
ars(300, f = function(x) dnorm(x,5), -10, 10, c(-1, 7))
ars(300, f = function(x) dgamma(x, 5), 0, 10, c(2,5))
ars(300, f = function(x) dbeta(x,2,2), 0, 1, c(0.1,0.6))
...
# timing
system.time(ars(1000, f = function(x) dnorm(x, 2), -Inf, Inf, c(-2, 3)))
 # user    system elapsed 
 # 12.966   0.140  13.283
```
