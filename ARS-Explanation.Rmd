...
### Explanation
Here are some assumptions that we began with. We assume that the the user will input valid density with correct minimum and maximum values of the domain. For example, if the input density is the standard normal density than the minimum and maximum values will be `-Inf` and `Inf`, respectively.

We also started with assuming that the starting points (2) will be provided, just to make computations easier. Then we relaxed those conditions to make the function work without starting points input. 
...
Although we divided the task as below we all contributed to all the tasks to some extent by checking for errors and making codes faster/better.
